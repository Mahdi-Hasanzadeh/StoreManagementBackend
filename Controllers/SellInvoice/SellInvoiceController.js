...
export const getUnpaidInvoices = async (req, res) => {
  try {
    ...
    const unpaidInvoices = await SellInvoiceModel.find({
      user: userId,
      customer: customerId,
      remaining_amount: { $gt: 0 },
    })
      .select("remaining_amount customer")
      .populate({
        path: "customer",
        select: "name",
      });
...
export const payRemainingForAll = async (req, res) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  ...
  try {
    ...
    const invoices = await SellInvoiceModel.find({
      user: userId,
      customer: customer_id,
      remaining_amount: { $gt: 0 },
    })
    ...
    if (paymentsToInsert.length > 0) {
      await CustomerPaymentModel.insertMany(paymentsToInsert, { session });
    }
